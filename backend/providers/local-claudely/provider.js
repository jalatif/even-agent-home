...
export function createLocalClaudelyProvider(emit) {
    ...
    function ensureKey() {
        ...
        if (!apiKey) {
            console.warn("[local-claudely] No LITELLM_API_KEY found in env or Bitwarden");
        }
        return apiKey;
    }
    ...
    async function prompt(sessionId, text, cwd) {
        ...
        if (!key) {
            ...
            throw new Error("local-claudely: LITELLM_API_KEY not found. Set LITELLM_API_KEY env var or configure Bitwarden.");
        }

        const session = makeSession(sessionId);
        if (!sessionId) {
            sessionId = `local-claudely-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
        ...
        const args = buildArgs(text, session.cwd);
        const env = buildEnv();

        debugLog("local-claudely", "spawning", `claude ${args.join(" ")}`);
        ...
        proc.stdout.on("data", (chunk) => {
            ...
            if (lines.length > 0) console.log(`[local-claudely] stdout: ${lines.length} line(s) for ${sessionId}`);
                    ...
                    if (event.type === "assistant" && event.message?.content) {
                        for (const block of event.message.content) {
                            if (block.type === "text" && block.text) {
                                console.log(`[local-claudely] emit text_delta: "${block.text.slice(0, 80)}" to ${sessionId}`);
                        ...
                        console.log(`[local-claudely] result: ${event.stop_reason} (error=${event.is_error})`);
                        if (event.is_error) {
        ...
        return new Promise((resolvePromise) => {
            proc.on("close", (code) => {
                console.log(`[local-claudely] closed code=${code} busy=${session.busy} sess=${sessionId}`);
                session.busy = false;
                emit(sessionId, { type: "result", success: code === 0, text: "", provider: "local-claudely" });
                emit(sessionId, { type: "status", state: "idle" });
                resolvePromise({ sessionId, provider: "local-claudely" });
            });
    ...
    function getStatus(sessionId) {
        ...
        return { state: session.busy ? "busy" : "idle", provider: "local-claudely" };
    }
