...
export class ClaudeSession {
    ...
    async run(prompt, model, thinking) {
        ...
        const options = {
            ...
            permissionMode: this.yolo ? "bypassPermissions" : "acceptEdits",
    ...
    enqueue(prompt, model, thinking) {
        this.promptQueue.push({ prompt, model, thinking });
    ...
    dispatchNext() {
        ...
        this.run(next.prompt, next.model, next.thinking).catch(err => {
    ...
    processStreamEvent(msg) {
        ...
        if (event.type === "content_block_delta" &&
            ...
            this.partialText = (this.partialText || "") + event.delta.text;
