...
export function createAntigravityProvider(emit) {
    ...
    function buildArgs(text, agyUuid, model, thinking, yolo) {
        // `agy -p` consumes the first non-flag arg as the prompt.  All
        // boolean/value flags must come BEFORE `-p`, otherwise they are
        // treated as the prompt itself.  Order: optional
        // --dangerously-skip-permissions first, then --conversation / --model
        // flags, then `-p`, then the prompt.
        const args = [];
        if (yolo) {
    ...
    async function prompt(phoneSessionId, text, cwd, model, thinking, yolo) {
        ...
        sessionCaches.clear(); // Invalidate cache so new session appears immediately
        ...
        // If the model string lacks a suffix (e.g. cached without @default), try to resolve it
        if (model && !model.includes("@") && !model.includes("/")) {
            // Hardcode a basic list of known suffixes for claude models to gracefully upgrade legacy strings
            if (model.startsWith("claude-")) {
                finalModel = `${model}@default`; // most claudes use @default if omitted
            }
        }
        const args = buildArgs(text, agyUuid, finalModel, thinking, yolo);
        ...
        if (thinking) {
            env.AGY_MODEL_TIER = thinking; // minimal, low, medium, high, xhigh
            // also try other common env vars just in case
            env.ANTIGRAVITY_MODEL_TIER = thinking;
            env.MODEL_TIER = thinking;
        }
        ...
        return new Promise((resolvePromise) => {
            ...
            const resolveTimer = setTimeout(resolvePrompt, 100);
            ...
            const finalize = (code) => {
                ...
                if (!session.agyUuid) {
                    ...
                    if (brainDir) {
                        ...
                        if (phoneSessionId) setBoundedMapping(phoneToAgy, phoneSessionId, uuid);
                        // Also map emitId so frontend can poll with the temp ID after process exits
                        if (emitId !== uuid) setBoundedMapping(phoneToAgy, emitId, uuid);
                    }
                }
    ...
    function transcriptStatus(dir) {
        ...
        try {
            ...
            if (last.type === "TURN_END" || last.type === "AGENT_END") return "idle";
            if (last.type === "USER_INPUT" || last.type === "TOOL_RESULT") return "busy";
            if (last.type === "ASSISTANT_TEXT" && last.complete === false) return "busy";
            return "idle";
        } catch {
            return null;
        }
    ...
    function listSessionsForCwd(cwd) {
        // Build a set of brain-dir UUIDs known to belong to this cwd by
        // correlating history.jsonl workspace entries with brain dir mtimes.
        // agy doesn't store workspace per brain dir, so we match by mtime
        // proximity (within 10s of a history entry for this workspace).
        ...
        for (const entry of history) {
            ...
            try {
                ...
                if (!isNaN(ts)) workspaceTimestamps.push(ts);
            } catch {}
        }
        ...
        for (const dir of allDirs) {
            ...
            let mtime;
            try { mtime = statSync(transcript).mtime; } catch { mtime = new Date(0); }
            // If we have workspace timestamps from history.jsonl, only include
            // brain dirs whose mtime correlates with a history entry for this
            // workspace. If cwd was specified but no matching history entries
            // exist (new workspace with no history), return empty.
            if (workspaceTimestamps.length > 0) {
                const matches = workspaceTimestamps.some(ts => Math.abs(ts - mtime.getTime()) < 10000);
                if (!matches) continue;
            } else if (cwd) {
                // cwd was provided, but no history entries exist for it — skip
                continue;
            }
    ...
    function listSessions(limit, cwd) {
        ...
        for (const [id, session] of sessions.entries()) {
            if (cwd && session.cwd && session.cwd !== cwd) continue;
            if (session.agyUuid && data.some(s => s.id === session.agyUuid)) continue;
            if (data.some(s => s.id === id)) continue;
            
            data.push({
                id: id,
                title: session.partialText ? session.partialText.slice(0, 64) : "New Session",
    ...
    function getHistory(sessionId, limit) {
        ...
        for (const e of events) {
            if (e.type === "USER_INPUT" && typeof e.content === "string") {
                ...
                partialText = ""; // new turn
            } else if (e.type === "PLANNER_RESPONSE" && typeof e.content === "string") {
                history.push({ role: "assistant", text: e.content });
                partialText = ""; // finished turn
            } else if (e.tool_calls && e.tool_calls.length > 0) {
                // If there are tool calls, append their summaries to partial text
                for (const call of e.tool_calls) {
                    if (call.toolSummary) {
                        partialText += `[${call.toolSummary}] `;
                    } else if (call.name) {
                        partialText += `[Running ${call.name}] `;
                    }
                }
            }
        }
        ...
        if (s && s.busy && s.partialText) {
            ...
            const isBusy = lastEvent.type !== "PLANNER_RESPONSE" && lastEvent.type !== "ERROR";
            if (isBusy) {
                history.push({ role: "assistant", text: "Thinking... " + partialText });
            }
        }
    ...
    function interrupt(sessionId) {
        ...
        procBySession.delete(sessionId);
        const s = getSession(sessionId);
        if (s) {
            s.busy = false;
            s.proc = null;
    ...
    function dispose() {
        for (const proc of procBySession.values()) {
            if (proc && !proc.killed) {
