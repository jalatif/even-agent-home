...
export function createOhMyPiProvider(emit) {
    ...
    /** Read the first user message from an omp session JSONL file.
 *  The session header may not have a meaningful title, so we walk
 *  through the first 32 KB looking for the first user message. */
        ...
        for (const file of files) {
            try {
                ...
                if (!title) {
                    ...
                    title = firstMsg ? firstMsg.slice(0, 64) : `omp-${sessionEvent.id.split('-')[0]}`;
                }
                result.push({
                    id: sessionEvent.id,
                    title,
