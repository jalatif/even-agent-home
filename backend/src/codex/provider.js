...
export function createCodexProvider(emit, getClient) {
    ...
    function dispatchNext(sessionId) {
        ...
        session.run(next.text, next.model, next.thinking).catch(err => {
    ...
    function observeThread(threadId, cwd, status = "idle") {
        if (!threadId)
            return null;
        let session = sessions.get(threadId);
        if (!session) {
            console.log(`[codex-provider] Observing listed thread ${threadId}`);
            session = new CodexSession(wrappedEmit, client);
            sessions.set(threadId, session);
            session.start(threadId, cwd).catch(() => { });
        }
        else if (cwd && !session.cwd) {
            session.start(threadId, cwd).catch(() => { });
        }
        session.syncObservedStatus(status);
    ...
    async function getSessionStatus(sessionId) {
        ...
        try {
            ...
            if (status === "busy" || status === "awaiting") {
                observeThread(sessionId, thread?.cwd, status);
            }
            return status;
        }
        catch {
    ...
    async function listSessions(limit, cwd) {
        const listedSessions = await listCodexSessions(client, limit, cwd);
        for (const s of listedSessions) {
            if (s.status === "busy" || s.status === "awaiting") {
                observeThread(s.id, s.cwd || cwd, s.status);
            }
        }
        return listedSessions.map((s) => {
            const observed = sessions.get(s.id);
            return { ...s, provider: "codex", status: observed?.status ?? s.status ?? null };
        });
    }
    ...
    async function getHistory(sessionId, limit) {
        ...
        const history = await getCodexSessionHistory(client, sessionId, limit);
        const observedSession = sessions.get(sessionId);
        if (observedSession && observedSession.busy && observedSession.assistantText) {
            history.push({ role: "assistant", text: observedSession.assistantText });
        }
